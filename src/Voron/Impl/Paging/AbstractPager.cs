...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        protected AbstractPager(RavenLogger logger, StorageEnvironmentOptions options, bool canPrefetchAhead, bool usePageProtection = false) : this()
        {
            Logger = logger;
        ...
        ~AbstractPager()
        {
            ...
            catch (Exception e)
            {
                try
                {
                    if (Logger.IsWarnEnabled)
                        Logger.Warn("AbstractPager finalizer was called (leak?), and 'DisposeOnceRunner.Dispose' threw exception", e);
                }
                catch
                {
                    // ignore
            ...
            finally
            {
                ...
                {
                    if (Logger.IsWarnEnabled)
                        Logger.Warn("AbstractPager finalizer was called although GC.SuppressFinalize should have been called", new InvalidOperationException("Leak in "));
                }
                catch
                {
                    // ignore
