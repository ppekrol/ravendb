        ...
        private async Task SendAsync(string command, string value, string[] values)
        {
            ...
            try
            {
                ...
                await using (var writer = new AsyncBlittableJsonTextWriter(context, _ms))
                {
                    ...
                    await writer.WriteIntegerAsync(currentCommandId).ConfigureAwait(false);

                    await writer.WriteCommaAsync().ConfigureAwait(false);
                    await writer.WritePropertyNameAsync("Command").ConfigureAwait(false);
                    await writer.WriteStringAsync(command).ConfigureAwait(false);
                    await writer.WriteCommaAsync().ConfigureAwait(false);

                    await writer.WritePropertyNameAsync("Param").ConfigureAwait(false);
                    await writer.WriteStringAsync(value).ConfigureAwait(false);

                    if (values != null && values.Length > 0)
                    {
                        await writer.WriteCommaAsync().ConfigureAwait(false);
                        await writer.WriteArrayAsync("Params", values).ConfigureAwait(false);
                    }
