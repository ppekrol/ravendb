...
namespace Raven.Embedded
{
    public class EmbeddedServer : IDisposable
    {
        ...
        private readonly Logger _logger = RavenLogManager.Instance.GetLoggerForEmbedded<EmbeddedServer>();
        ...
        private void ShutdownServerProcess(Process process)
        {
            ...
            lock (process)
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        _logger.Warn(e, $"Failed to shutdown server PID {process.Id} gracefully in {_serverOptions!.GracefulShutdownTimeout}");
                    }
                }
                ...
                catch (Exception e)
                {
                    if (_logger.IsWarnEnabled)
                    {
                        _logger.Warn(e, $"Failed to kill process {process.Id}");
                    }
                }
            }
        }
