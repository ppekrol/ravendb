        ...
        private async Task InstallUpdatedCertificateValueChanged(long index, string type)
        {
            try
            {
                ...
                using (context.OpenReadTransaction())
                {
                    ...
                    var certificate = CertificateLoaderUtil.CreateCertificateWithoutPrivateKey(Convert.FromBase64String(base64Cert));
        ...
        private async Task ConfirmCertificateReceiptValueChanged(long index, string type)
        {
            try
            {
                using (ContextPool.AllocateOperationContext(out TransactionOperationContext context))
                {
                    ...
                    var newClusterCertificate = CertificateLoaderUtil.CreateCertificateWithPrivateKey(bytesToSave, password: null, flags: CertificateLoaderUtil.FlagsForExport);
