...
namespace Raven.Server.Utils.Cli
{
    public sealed class RavenCli
    {
        ...
        private static bool CommandTrustServerCert(List<string> args, RavenCli cli)
        {
            ...
            try
            {
                cert = CertificateLoaderUtil.CreateCertificateWithPrivateKey(path, args.Count == 3 ? args[2] : null, flags: null);
            }
            catch (Exception e)
            {
        ...
        private static bool CommandTrustClientCert(List<string> args, RavenCli cli)
        {
            ...
            try
            {
                ...
                cert = CertificateLoaderUtil.CreateCertificateWithPrivateKey(certBytes, password, flags: null);
            }
            catch (Exception e)
            {
                WriteError("Failed to load the provided certificate. Please check the path and password." + e, cli);
        ...
        private static bool CommandReplaceClusterCert(List<string> args, RavenCli cli)
        {
            ...
            {
                certBytes = File.ReadAllBytes(path);
                CertificateLoaderUtil.ImportWithPrivateKey(cert, certBytes, password, CertificateLoaderUtil.FlagsForExport);
            }
            catch (Exception e)
            {
                WriteError("Failed to load the provided certificate. Please check the path and password." + e, cli);
            ...
            try
            {
                loadedCert = CertificateLoaderUtil.CreateCertificateWithPrivateKey(certBytes, password: null, flags: CertificateLoaderUtil.FlagsForPersist);
            }
            catch (Exception e)
            {
