...
namespace Raven.Server.Documents
{
    public class CatastrophicFailureHandler
    {
        ...
        private static readonly Logger Logger = RavenLogManager.Instance.GetLoggerForServer<CatastrophicFailureHandler>();
        ...
        public void Execute(string databaseName, Exception e, Guid environmentId, string path, string stacktrace)
        {
            ...
            stats.DatabaseUnloadTask = Task.Run(async () =>
            {
                ...
                if (Logger.IsFatalEnabled)
                    Logger.Fatal(e, $"{title}. {message} (StackTrace='{stacktrace}')");
