...
namespace Raven.Server.Documents.PeriodicBackup.Aws
{
    public class RavenAwsS3Client : IDisposable
    {
        ...
        public RavenAwsS3Client(S3Settings s3Settings, Progress progress = null, CancellationToken cancellationToken = default)
        {
            ...
            _client = new AmazonS3Client(new BasicAWSCredentials(s3Settings.AwsAccessKey, s3Settings.AwsSecretKey), config);
        ...
        public void PutObject(string key, Stream stream, Dictionary<string, string> metadata)
        {
            ...
            try
            {
                ...
                if (streamSize > MaxUploadPutObject)
                {
                    ...
                    var initiateResponse = _client.InitiateMultipartUploadAsync(multipartRequest, _cancellationToken).GetAwaiter().GetResult();
                    var partNumber = 1;
                    ...
                    _client.CompleteMultipartUploadAsync(
                ...
                _client.PutObjectAsync(request, _cancellationToken).GetAwaiter().GetResult();
            }
        ...
        {
            var aclResponse = _client.GetACLAsync(_bucketName, _cancellationToken).GetAwaiter().GetResult();
        ...
        {
            var bucketLocationResponse = _client.GetBucketLocationAsync(_bucketName, _cancellationToken).GetAwaiter().GetResult();
        ...
        private void MaybeHandleException(AmazonS3Exception exception)
        {
