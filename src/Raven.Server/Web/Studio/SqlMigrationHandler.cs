    ...
    public class SqlMigrationHandler : DatabaseRequestHandler
    {
        ...
        public async Task SqlSchema()
        {
            ...
            using (var sourceSqlDatabaseBlittable = context.ReadForMemory(RequestBodyStream(), "source-database-info"))
            {
                ...
                {
                    await context.WriteAsync(writer, schema.ToJson());
                }
            }
        }
            ...
            using (ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                using (var sqlImportTestDoc = context.ReadForMemory(RequestBodyStream(), "sql-migration-test-request"))
                {
                    ...
                    await using (var writer = new AsyncBlittableJsonTextWriter(context, ResponseBodyStream()))
                    {
                        ...
                        await writer.WritePropertyNameAsync("DocumentId");
                        await writer.WriteStringAsync(documentId);

                        await writer.WriteCommaAsync();

                        await writer.WritePropertyNameAsync("Document");
                        await writer.WriteObjectAsync(testResultDocument);
